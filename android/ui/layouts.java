...
/*
	ListView. (https://developer.android.com/guide/topics/ui/declaring-layout.html#AdapterViews)

	Отображает список прокручиваемых элементов. Элементы списка вставляются автоматически с помощью Adapter или CursorLoader (по поводу CursorLoader это не точно), который берёт данные из источника и создаёт на их основе элемент списка.

	CursorLoader это инструмент для выполнения асинхронного запроса, который позволяет избежать блокировки основного потока в момент запроса.

	Adapter для создания ListView.

	Если содержимое макета является динамическим можно использовать макет, который создает подклассы класса AdapterView.
	...
	В AdapterView входят ListView и GridView.

	Вот два наиболее часто используемых адаптера:

		- ArrayAdapter. (Этот адаптер используется в случае, когда в качестве источника данных выступает массив. По умолчанию ArrayAdapter создает представление для каждого элемента массива путем вызова метода toString() для каждого элемента и помещения его содержимого в объект TextView)

		- SimpleCursorAdapter. (Этот адаптер используется в случае, когда в качестве источника данных выступает объект Cursor. Если используется SimpleCursorAdapter, необходимо указать макет, который будет использоваться для каждой строки в объекте Cursor, а также в какие представления макета необходимо вставить столбцы в объекте Cursor)

	Чтобы организовать реагирование на нажатие каждого элемента в AdapterView, реализуйте интерфейс AdapterView.OnItemClickListener.

	Элементы в списке могут быть произвольными объектами Java.

	Adapter расширяет BaseAdapterкласс.
*/